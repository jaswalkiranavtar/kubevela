...
var _ = Describe("Test Application Controller", func() {
	...
	It("test application with ocm-topology policy creates ManifestWork instead of rendering normally", func() {
		// Create test namespace for the application
		testNs := &corev1.Namespace{
			ObjectMeta: metav1.ObjectMeta{
				Name: "vela-test-ocm-topology",
			},
		}
		Expect(k8sClient.Create(ctx, testNs)).Should(BeNil())

		// Create namespace for the managed cluster (ManifestWork will be created here)
		managedClusterNs := &corev1.Namespace{
			ObjectMeta: metav1.ObjectMeta{
				Name: "test-managed-cluster",
			},
		}
		Expect(k8sClient.Create(ctx, managedClusterNs)).Should(BeNil())
		...
		managedCluster := &clusterv1.ManagedCluster{
			...
			Spec: clusterv1.ManagedClusterSpec{
				...
				ManagedClusterClientConfigs: []clusterv1.ClientConfig{
					{URL: "https://test-managed-cluster.example.com"},
				},
			},
		}
		Expect(k8sClient.Create(ctx, managedCluster)).Should(BeNil())

		// Create Application with ocm-topology policy
		app := &v1beta1.Application{
			TypeMeta: metav1.TypeMeta{
				Kind:       "Application",
				APIVersion: "core.oam.dev/v1beta1",
				...
				Policies: []v1beta1.AppPolicy{
					{
						...
						Properties: &runtime.RawExtension{
							Raw: []byte(`{"clusters":["test-managed-cluster"]}`),
						},
					},
				},
			},
		...
		By("Verifying application status")
		curApp := &v1beta1.Application{}
		Expect(k8sClient.Get(ctx, appKey, curApp)).Should(BeNil())
		// The app should have workflow steps generated (deploy-ocm)
		Expect(curApp.Status.Workflow).ShouldNot(BeNil())

		By("Verifying NO Deployment is created in the application namespace (should not render normally)")
		deploymentList := &v1.DeploymentList{}
		Expect(k8sClient.List(ctx, deploymentList, client.InNamespace(testNs.Name))).Should(BeNil())
		Expect(len(deploymentList.Items)).Should(Equal(0), "No Deployment should be created when ocm-topology policy is present")

		By("Verifying ManifestWork is created in the managed cluster namespace")
		manifestWorkList := &ocmworkv1.ManifestWorkList{}
		Eventually(func() int {
			err := k8sClient.List(ctx, manifestWorkList, client.InNamespace(managedClusterNs.Name))
			if err != nil {
				return 0
			}
			return len(manifestWorkList.Items)
		}, 10*time.Second, 500*time.Millisecond).Should(BeNumerically(">=", 1), "ManifestWork should be created in managed cluster namespace")

		By("Verifying ManifestWork contains valid Application with apiVersion and kind")
		mw := &manifestWorkList.Items[0]
		Expect(len(mw.Spec.Workload.Manifests)).Should(BeNumerically(">=", 1))
		rawManifest := mw.Spec.Workload.Manifests[0].Raw
		Expect(string(rawManifest)).Should(ContainSubstring(`"apiVersion":"core.oam.dev/v1beta1"`))
		Expect(string(rawManifest)).Should(ContainSubstring(`"kind":"Application"`))

		By("Verifying ManifestWork labels")
		Expect(mw.Labels["app.oam.dev/name"]).Should(Equal(app.Name))
		Expect(mw.Labels["app.oam.dev/namespace"]).Should(Equal(app.Namespace))

		By("Cleanup")
		Expect(k8sClient.Delete(ctx, app)).Should(BeNil())
		Expect(k8sClient.Delete(ctx, managedCluster)).Should(BeNil())
	})
})
